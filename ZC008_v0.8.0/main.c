...
volatile WindowType Apply_WindowType = 0; //默认使用矩形窗
...
int main(void)
{
    ...
    while(1)
    {
        ...
        if (UART0_RxSTA & 0x8000)
        {
            ...
            //查看FFT结果
            if (UART0_len > 0 && UART0_RxBuff[0] == 3)
            {
                delay_ms(20);
                printf("page result\xff\xff\xff");
                delay_ms(500);
                DisplayResults();
            }

            //切换窗函数
            if (UART0_len > 0 && UART0_RxBuff[0] == 4)
            {
                delay_ms(20);
                Apply_WindowType ++;
                if (Apply_WindowType > 4) Apply_WindowType = 0;
                switch(Apply_WindowType)
                {
                    case WINDOW_RECTANGULAR:
                        printf("result.t8.txt=\"Rectangular\"\xff\xff\xff");
                        break;
                    case WINDOW_HANNING:
                        printf("result.t8.txt=\"Hanning\"\xff\xff\xff");
                        break;
                    case WINDOW_HAMMING:
                        printf("result.t8.txt=\"Hamming\"\xff\xff\xff");
                        break;
                    case WINDOW_FLATTOP:
                        printf("result.t8.txt=\"Flattop\"\xff\xff\xff");
                        break;
                    case WINDOW_BLACKMAN:
                        printf("result.t8.txt=\"Blackman\"\xff\xff\xff");
                        break;
                    default:
                        break;
                }
        ...
        if (ADC_BusyFlag == 0 && TimerADC_BusyFlag == 1)
        {
            ...
            {
                if (FinalSample_Flag == 1) //最终数据处理
                {
                    FinalSample_Flag = 0;
                    Freq = Calculate_Freq_Final();
                }
                else //最终采样
                {
                    uint16_t LV_temp = (CPUCLK_FREQ / (Freq * 193.0)) - 1; //选取合适的自动重装值以恰好采样5个周期(193=1024/5.3)
                    if (LV_temp >= 79U && LV_temp <= 65519U) //在合理范围内
                    {
                        LoadValue = LV_temp;
                        DL_Timer_setLoadValue(TIMER_ADC0_INST, LoadValue); //更改自动重装值
                    }
