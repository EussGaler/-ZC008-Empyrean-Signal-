...
uint16_t ADC_Trigger = 1000; //触发电平
uint16_t ValidPairs[TRIG_LENGTH][2]; //有效周期触发点对
uint16_t PeriodCount = 0; //有效周期数
...
//在最终调节采样后，计算频率并查找电平跳变点
uint32_t Calculate_Freq_Final(void)
{
    //均值滤波
    Smooth_Filter((uint16_t *)ADC_Value0, (uint16_t *)ADC_Value1, (uint16_t)ADC_LENGTH);

    //确定触发电平
    uint16_t v_max = 0, v_min = 0;
    for (uint16_t i = 0; i < ADC_LENGTH; i++)
    {
        if (ADC_Value1[i] > v_max)
            v_max = ADC_Value1[i];
        if (ADC_Value1[i] < v_min)
            v_min = ADC_Value1[i];
    }
    ADC_Trigger = (v_max + v_min) / 2;
    ...
    //进行最终数据处理
    Freq = Calculate_Final();
    PeriodCount = Find_ValidTriggerPairs(TriggerIndex, TriggerCount, ValidPairs);
    // __BKPT(0);
    DoFFT();
...
/**
 * @brief 找出有效的触发点对
 * @param TrigIdx 触发点索引数组
 * @param n 触发点数量
 * @param validPairs 存储有效点对的二维数组
 * @return 有效点对的数量
 */
uint16_t Find_ValidTriggerPairs(uint16_t *TrigIdx, uint16_t n, uint16_t (*validPairs)[2])
{
    if (n < 2) return 0; //至少需要2个点才能形成点对

    //计算所有相邻点之间的间距
    uint16_t *diffs = (uint16_t *)malloc((n - 1) * sizeof(uint16_t));
    for (uint16_t i = 0; i < n - 1; i++)
        diffs[i] = TrigIdx[i + 1] - TrigIdx[i];

    //计算间距的中位数
    uint16_t median_diff;
    qsort(diffs, n - 1, sizeof(uint16_t), compare); //排序
    
    if ((n - 1) % 2 == 1) //奇数个元素
        median_diff = diffs[(n - 1) / 2];
    else //偶数个元素
        median_diff = (diffs[(n - 2) / 2] + diffs[n / 2]) / 2;

    //设定阈值
    uint16_t max_threshold = median_diff * 1.1;
    uint16_t min_threshold = median_diff * 0.9;
    uint16_t validCount = 0;

    //筛选有效的点对
    for (uint16_t i = 0; i < n - 1; i++)
    {
        uint16_t current_diff = TriggerIndex[i + 1] - TriggerIndex[i];
        
        //只保留大于阈值的点对
        if (current_diff >= min_threshold && current_diff <= max_threshold)
        {
            validPairs[validCount][0] = TriggerIndex[i];
            validPairs[validCount][1] = TriggerIndex[i + 1];
            validCount++;
        }
    }
